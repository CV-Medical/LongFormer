...
def collate_fn(batch):
    ...
    batch[0] = torch.stack(batch[0]).unsqueeze(2) # b t c h w d
    return tuple(batch)
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], size_divisibility=1, split=True):
    ...
    if tensor_list[0].ndim == 4:
        ...
        if size_divisibility > 1:
            ...
            # the last 3 dims are D,H,W, both subject to divisibility requirement
            max_size[-3] = (max_size[-3] + (stride - 1)) // stride * stride
